...
| Builder (Today step 3: badges, pointer, callouts) | `static/js/meridian/dial.js`, `static/css/meridian/dial.css`, `tests/meridian/test_dial_js.py`, `docs/project/CURRENT_STATUS.md`, `docs/project/AGENT_COORDINATION.md`, `docs/project/MERIDIAN_OS_TASKS.json`, `design-qa.md` | 2026-09-16 | active. Handoff step 3, first batch: semantic kit glyphs per event, brass-rimmed weighted badges, prominent mint pointer, and the diagnosed pre-existing mobile callout word-split defect. **No** connector geometry, ticket art, routes, data, financial or authority change. |
