...
|---|---|---|---|
| DeepSeek (OS-048a bill→reserve membership) | `meridian/providers/{base,crewwork}.py`, `meridian/{commitments,sync,live}.py`, `meridian/services/dial.py`, `meridian/migrations/023_commitment_bill_reserve.sql` (new), `docs/project/shipped-migrations.json`, `static/js/meridian/dial.js`, `tests/meridian/{test_migrations.py,providers/test_crewwork.py,test_dial_js.py,test_dial_income_meta.py,test_commitments.py}`, `tests/meridian/services/test_dial.py`, focused new test files, status/coordination/claims/task records | 2026-09-19 | **Active.** Bounded read-side slice per D-010: retain the observed bill→bill-reserve membership during ingestion, persist it, and expose funding-source identity separately from occurrence reservation status. No provider mutation, no live sync, no backfilled membership, no occurrence reserved amounts. Note the recon finding that `sync_providers` has no production caller while `live.py::sync_live_crew` duplicates the candidate→commitment mapping — both sites must change. |
...
### 2026-09-19 — OS-048a: the bill→reserve membership survives ingestion, and the dial names the funder
