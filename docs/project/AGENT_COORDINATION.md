...
| Builder (Plan: folded allocation map) | `templates/meridian/partials/plan.html`, `static/js/meridian/plan.js`, `static/css/meridian/plan.css`, `tests/meridian/test_plan_map.py` (new), `tests/browser/test_plan.py`, `docs/project/CURRENT_STATUS.md`, `docs/project/AGENT_COORDINATION.md`, `docs/project/MERIDIAN_OS_TASKS.json`, `design-qa.md` | 2026-09-16 | active. Track D Plan, first batch: the kit's folded-map allocation replaces the generic stacked bar and swatch legend and is installed below the tabs. Stations are composition, not amounts; every figure stays HTML. **No** route, data, financial or authority change. |
