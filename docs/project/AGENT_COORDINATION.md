...
|---|---|---|---|
| Codex OS-116 planning | `docs/superpowers/specs/2026-09-26-reserve-timeline-design.md`, `docs/superpowers/plans/2026-09-26-reserve-timeline.md`, `design/reserve-timeline-2026-09-26/`, `docs/project/{CURRENT_STATUS.md,MERIDIAN_OS_TASKS.json,MERIDIAN_DECISIONS.md,session-emergent.json,HANDOFF.md,AGENT_COORDINATION.md}` | 2026-09-26 | active; design/documentation only |
...
| 2026-09-25 (evening) | constitutional-builder | OS-106 shell markup guard; OS-107/OS-108 Assets & Contracts (per-item tickets, kit button plates, serif on paper, sentence labels); OS-109 stylus to the centre; OS-110 readout clear of the rotunda | complete; whole suite green; commits 24d0134, 4367dcb, 2f36ffe |

| 2026-09-25 (verification pass) | independent-verifier (adversary role) | Re-derive FIVE claims by an instrument the original did not use — (1) capability 17/17 with carriers + 16/17 with verifiers, by EXECUTING the pipeline against a scratch DB; (2) the ungoverned write surface = exactly the 26 declared routes, by HITTING the routes through the app's HTTP surface with the provider stubbed, never by AST; (3) Today and Plan publish one money rule, through the HTTP surface not the unit fixture; (4) the allocation ordering rule, against the raw provider read not the document; (5) the orphan list = exactly one module, by RUNTIME import tracing not grep | in progress; verifier writes NO application code, scratch work in /tmp only, no repo writes beyond this row and the report; classifies each CONFIRMED / FALSIFIED / COULD-NOT-CHECK with the instrument named |
