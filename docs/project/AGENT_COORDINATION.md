...
| Builder (Activity follow-up: light green, row glyphs, review count) | `static/js/meridian/kit-icons.js`, `static/js/meridian/activity.js`, `static/css/meridian/activity.css`, `templates/meridian/partials/activity.html`, `meridian/repository.py`, `meridian/services/activity.py`, `meridian/api.py`, `tests/meridian/test_activity_vignette.py`, `tests/meridian/test_activity_glyph.py`, `tests/meridian/test_activity_review_count.py` (new), `tests/browser/test_activity.py`, `artifacts/observatory-activity-followup-2026-09-18/**` (untracked by convention), `docs/project/CURRENT_STATUS.md`, `docs/project/AGENT_COORDINATION.md`, `docs/project/MERIDIAN_OS_TASKS.json`, `design-qa.md` | 2026-09-18 | **active**. Owner-directed follow-up from live-device review: the light edition's action colour becomes the palette's existing green (not a second brand colour); the unconfirmable review control stops reading as a dead greyed button; the category glyph stops rendering blank for `bank` and stops degenerating to `?` for recognised merchants; and the previously-blocked "N categories to review" strip is built on a **new read-only count**, which the owner explicitly authorised. `meridian/repository.py`/`api.py` gain a read-only aggregate only -- no classification, write, provider or authority change. |
