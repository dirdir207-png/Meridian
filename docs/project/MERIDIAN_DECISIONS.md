...
**Where the third label came from (added 2026-09-25, from the owner's own question).** The owner asked whether OS-119's second write surface descends from the early era when parity was attempted by "having the mac crew app driven". Tracing it: `docs/project/CREW_GRAPHQL_CATALOG.md:3-8` records that the operation catalog was captured 2026-09-07 from the live native app (`com.trycrew.crew`, React Native/Expo v2.2.7) by routing its HTTPS through mitmproxy and **driving it via cliclick** — a one-time observation technique to obtain the real contracts, never a way of executing writes. That capture is where the label `ArchiveBill` ("bill delete/archive", input `{billId}`) enters this repository, and `docs/project/crew_mutations.json` inherited it; our `meridian/crew_commands.py` carries the same operation as `DeleteBill`, and the connector's CLI operation is `archive_bill`. **Three labels, one operation** — which is why a name-keyed comparison was guaranteed to manufacture a phantom gap, and why D-039's rule is to key on the name the provider actually receives. (The catalog also lists `InitiateTransferRuth` where `meridian/crew_write.py` carries `InitiateTransferScottie` for the same shape, so the provider's own mutation names appear to differ by client build; that is recorded as an observation, not as a conclusion — resolving it needs a live read, which this lane does not perform for a naming question.)
