...
**Dates can move without the owner — never attribute a change to him (2026-09-20).** The Rent history shows why. Rent is normally due on the **5th**; the owner changed it to the **16th**; then his **direct deposit switched to Crew**, which moved it to the **18th** — in his words, *"a correction I didn't make."* A bill's date is therefore **neither necessarily owner-set nor stable**: the system itself can revise it. Three rules follow: never assume a stored due date was the owner's choice; never present a moved date as an anomaly, an error, or drift to be corrected; and when Meridian explains a change, state **what it observed** rather than **who caused it**, because it cannot know that.
