...
Treat visual fidelity as a formal verification track governed by `docs/project/MERIDIAN_VISUAL_CAPTURE_SPEC.md`. Before changing UI, use that capture contract and create or update a baseline manifest containing concept path, exact viewport, DPR, theme, fixture data, clock, current capture, UI state, fullPage setting, commit, and capture date. Required viewports are 1440×900 and 1024×768 at DPR 1, plus 430×932 and 390×844 at DPR 3. Capture light and dark explicitly. Use deterministic synthetic data, frozen time, loaded fonts, disabled motion, disabled polling/timers/auto-refresh, and settled network/data requests. Capture both initial viewport and defined full-page artifacts. Never compare a mobile concept with a desktop capture. Preserve aspect ratios. Generate side-by-side and overlay/difference views where possible. Review structure/landmarks first, then typography, spacing, controls/accessibility, and decoration. Change one visual gap at a time, recapture, and record the result. Validate live-data behavior separately from concept fidelity.
