...
### Parked entries
| **OS-116** | **Reserve Timeline** — a Meridian-native ordered cash-flow timeline, explicitly NOT a copy of Crew's Plan Reserve page. Balance chart with running balance, multiple ordered events per date, event TYPES kept visibly separate (income, bill obligation, pocket transfer, observed reserve, projection, shortfall), expandable grouped rows that sum exactly to their group, provenance badges for Crew-reported / Crew-estimate / Meridian projection / simulated, and a SEPARATE "current reserve allocation" view sourced from the dated history so forecast cashflow is never confused with Crew's observed `reservedAmount`. Owner-supplied brief 2026-09-25, with a verified trap attached: Crew's group label "Allocation Probe $300 First + 7 more" heads **8 obligations totalling 815.00** (verified against the live database), so a screen label naming one bill is never evidence that the bill owns the reserve. | **Track D surfaces, AFTER a Meridian design pass produces three visual directions and the owner selects one, followed by an implementation plan** — the owner's own sequence: *"the next safe step is a Meridian design pass with three visual directions, followed by a selected design and implementation plan before touching code."* | **Parked, not assigned, and visual authority is not this lane's.** The owner is awaiting a usage reset and design instructions from Astra; designing it unilaterally would be exactly the visual decision that has bitten before. A data-gap review IS recorded with the entry, because a design that promises data we lack is the failure mode to avoid: the funding EVENT has no observation source (only the funding plan), pocket transfers are not modelled and the pockets facet exposes no account-level balance, and a running balance needs every movement including the reserve's. Also a hard requirement, not a nicety: the temporary `Allocation Probe*` bills and the injected 480.77 reserve are TEST FIXTURES and must not drive the production UI (D-027). |
...
| **OS-084** | **Every route built on `_management_payload` returns a proposal  `_meridian_memory_proposal_sink` (`app.py:1299-1317`) calls `action_store.propose(...)` and never consults the  so bills, rules, assets and contracts park the owner's own unambiguous in-app edits for approval. **D-020 records the rule** that a direct, fully-specified owner edit executes instead. | **After `OS-083` proves the pattern narrow.** Payday is the first surface routed the right  `POST /api/actions/mutate` with `provenance: owner_direct`, and deliberately NO parallel proposal-only  and its end-to-end test is the template to copy: direct reaches the executor, interpreted and low-confidence still propose. Converting four more surfaces before one is proven would repeat the same mistake at scale. | Not a design choice but a **defect against D-020**, and the owner has chosen narrow-first deliberately. The cost of waiting is contained: those surfaces still work, they simply ask for an approval they should not need. |
...
| **Desktop Settings hub** | At >=901px the eight hub rows and their group banners sit in the narrow settings rail and wrap onto four or five lines, while the main column holds only an intro paragraph. | **Track D design-fidelity close-out (`OS-038`).** The desktop treatment of a concept drawn as a phone frame is a design decision, not a bug fix, and `OS-038` is where the remaining concept-vs-current visual gaps are reconciled. | Measured IDENTICAL before and after the 2026-09-21 stylesheet fix, so it dates from `OS-065` and is NOT a regression. The 09-18 concept specifies no desktop treatment for Settings, so there is nothing to compare against -- ****the authority already specifies this, so it is NOT awaiting the owner** -- the earlier claim that the concept specifies no desktop treatment was WRONG. CORRECTED 2026-09-21 by reading the record I had cited without reading: design/observatory-extension-2026-09-18/BUILD_HANDOFF.md line 24 states 'At 1440px: existing rail, flexible ledger, 320-360px inspector; Virgil opens there without adding a fifth workspace. SETTINGS USES GROUPED NAVIGATION LEFT AND SELECTED DETAILS RIGHT.' That is a specific, citable desktop treatment, and the same line gives the other widths. So the desktop hub is reconciled against 'grouped navigation left, selected details right' at 1440px -- not invented, and not a question to put to the owner**, and inventing a layout unilaterally would be a design decision recorded as a fix. |
