...
Must not be assumed: **no live sync, no provider call, no guessed backfill and no deployment.** The reserve total and the reported flag are written by the next ordinary read (the app's own refresh), so until that read happens the running app still states nothing new; `crew_bill_reserves` is empty in every existing database. The six pre-existing `tests/browser/test_dial_fidelity.py` failures (dial-wrap vs rail geometry; theme-toggle label width) were reproduced at pristine `2f2e833` with this diff stashed, so this slice does not cause them — they were already failing. `sync_providers` and `sync_live_crew` now copy three fields twice; OS-053's divergence risk is larger, not smaller.
