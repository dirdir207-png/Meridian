...
### Two capture targets are not workspaces

`settings` and `virgil` are selectable capture targets that are deliberately **not** added to `WORKSPACES`. `settings` loads `/meridian/settings` (`--settings-query` selects `?section=…` or the empty string for the hub, the no-section landing page). `virgil` loads `/meridian?workspace=today` and then opens the advisor panel through the shell's real `window.advisorSetOpen` entry point, because Virgil is an overlay over a workspace rather than a fifth workspace — and BUILD_HANDOFF.md forbids adding a fifth. Record `--ui-state virgil-panel-open` so the manifest says which state was compared; comparing a panel-open capture against a closed-state concept would be a state mismatch.
