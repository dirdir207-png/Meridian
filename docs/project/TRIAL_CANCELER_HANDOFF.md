...
1. No browser extension or local bridge; no merchant website is driven. A provider adapter protocol and stubbed replay harness now exist; the default executor hard-stops unless an adapter is explicitly injected. The cancellation brief is now available as a read-only API response.
