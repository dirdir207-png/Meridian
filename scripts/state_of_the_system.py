...
def render() -> str:
    ...
    add(f"| governed actions with a readback verifier (a REGISTRATION count) | {len(cap['actions']) - len(cap['unverified_actions'])} |")
    add(f"| allowed operations with NO governed action | {len(cap['ops_without_action'])} |")
    add("")
    add("A registration count is not an every-attempt execution count. An independent lane drove the dispatch path and")
    add("found 16/17 actions dispatched with 15/16 verifiers REACHED when an approval carries no reviewed base_state")
    add("(`update_crew_bill` is refused before dispatch), and 17/17 with 16/16 when it does — never restate the row above as")
    add("every-attempt behaviour (`INDEPENDENT_VERIFICATION_2026-09-25.md` §Claim 1).")
    add("")
    ...
    add(f"| routes DECLARED to reach a raw Crew mutation (the declaration calls them POST) | {len(ungoverned)} |")
    add("")
    ...
    add("denominator for OS-119: the shape decision concerns exactly these routes. THE DECLARATION'S OWN COMMENT DOES NOT")
    add("RECONCILE and is not repeated here: it says nine from the 2026-09-25 audit plus seventeen found by the ratchet,")
    add("which would be 26, but the two blocks hold 9 and 16 while a third route sits outside both — nine plus seventeen")
    add("plus one is 27 against a list of 26 (found by the independent lane, `INDEPENDENT_VERIFICATION_2026-09-25.md`")
    add("§Claim 2). The LIST is authoritative; the prose is not. Two further measured facts about the denominator:")
    add("the ratchet scans `app.py` ONLY, so the Meridian blueprint's POST rules are structurally outside it (swept once")
    add("by the independent lane and empirically empty, but structurally open), and `/api/cards/<card_id>/sensitive` is")
    add("GET-only at runtime while standing in a set described as POST routes — so the row above is \"declared\", not")
    add("\"POST\", and the FINANCIAL count is smaller than the DECLARED count. Quote whichever the question is about, and")
    add("say which.")
    add("")
