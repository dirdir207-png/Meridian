        ...
        def normalise(s: str) -> list[str]:
            ...
            for line in s.splitlines():
                ...
                if line.startswith("- Commit:"):
                    # Keep "on <branch>"; drop everything volatile after it. The dirty-tree
                    # note describes the WORKING TREE at generation time, so it differs between
                    # two runs over identical content — and it is regenerated by whoever
                    # finishes the work, so treating it as staleness produces a permanently
                    # red check.
                    line = line.split(" — ")[0]
                if line:
                    out.append(line)
