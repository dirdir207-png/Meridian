...
export function apply(ctx, config) {
  ...
  ctx.on('agent/pre-step', async ({ agent, signal }, next) => {
    ...
    try {
      ...
      const text = [
        ...sections,
        'These are accepted project instructions, and they bind this task: they are requirements under the workspace\'s precedence rules — a closer file overrides a broader one — not background reading. Read the relevant file before acting, and follow it. If one of them conflicts with a later request, follow the instruction file and say so explicitly rather than silently choosing.',
      ].join(' ')
