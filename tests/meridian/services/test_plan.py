...
def test_plan_includes_explainable_beacon_forecast(env):
    graph, commitments, rules = env
    account = _synced_checking(graph, balance=500.0)
    for index in range(1, 6):
        transaction = graph.upsert_transaction(
            provider="crew",
            external_id=f"spend-{index}",
            account_id=account.id,
            amount=-10,
            occurred_at=f"2026-08-{20 + index:02d}T10:00:00Z",
            description="Daily spend",
            status="posted",
        )
        from meridian.classify import ClassificationInput, classify_deterministic

        graph.record_classification(
            transaction.id,
            classify_deterministic(
                ClassificationInput(
                    ...
                    None,
                    "checking",
                    f"2026-08-{20 + index:02d}T10:00:00Z",
                )
            ),
        )
