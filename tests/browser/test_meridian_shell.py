...
def test_desktop_shell_renders_rail_canvas_and_inspector():
    ...
    with playwright.sync_playwright() as browser_driver:
        ...
        assert int(round(float(columns[0].removesuffix("px")))) == 150
...
def test_desktop_shell_matches_editorial_atlas_hierarchy():
    """Fails when Meridian falls back to the compact engineering shell."""
    playwright = pytest.importorskip("playwright.sync_api")
    _setup_module()
    ...
    with playwright.sync_playwright() as browser_driver:
        ...
        rail = page.locator("[data-primary-nav]")
        brand = rail.locator("[data-meridian-wordmark]")
        headline = page.locator('[data-workspace-section="today"] [data-editorial-headline]')
        hero = page.locator('[data-workspace-section="today"] [data-today-hero]')

        assert rail.evaluate("node => Math.round(node.getBoundingClientRect().width)") == 150
        assert brand.is_visible()
        assert headline.is_visible()
        assert headline.evaluate(
...
def test_mobile_brand_and_dock_labels_remain_legible_on_light_surfaces():
    """Catches white desktop-rail text leaking onto the mobile light chrome."""
    playwright = pytest.importorskip("playwright.sync_api")
    _setup_module()
    ...
    with playwright.sync_playwright() as browser_driver:
        ...
        contrast = page.evaluate(
            r"""() => {
              ...
              const brand = topbar.querySelector('.m-brand');
              const dock = document.querySelector('[data-primary-nav]');
              const dockLabel = dock.querySelector('.m-nav-item');
              return { brand: ratio(brand, topbar), dock: ratio(dockLabel, dock) };
            }"""
        )

        assert contrast["brand"] >= 4.5
        assert contrast["dock"] >= 4.5
        browser.close()
