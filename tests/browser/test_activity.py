...
def _live_today_payload():
    return {
        "total_cash": {
            "amount": 2460.0,
            "currency": "USD",
            "by_currency": {"USD": 2460.0},
        },
        "safe_to_spend": {
            "amount": 684.0,
            "currency": "USD",
            "status": "available",
            "change_since_yesterday": 92.0,
            ...
            "inputs": {
                ...
                "known_obligations": {
                    "amount": 1776.0,
                    "currency": "USD",
                    "by_currency": {"USD": 1776.0},
                },
            },
        },
        "upcoming_events": [
            {"label": "Payday", "date": "2026-09-04", "amount": "+$2,800"},
            {"label": "Rent", "date": "2026-09-05", "amount": "−$1,800"},
            {"label": "Insurance", "date": "2026-09-07", "amount": "−$164"},
        ],
        "forecast": {
            "available": True,
            "runway_days": 17,
        ...
        "beacon": {
            ...
            "summary": "Verizon rose by $11",
            "detail": "The bill is still funded. This is the third increase in six months.",
        },
        "brief": {
            "title": "A useful connection",
            "summary": "The September insurance renewal and travel deposit overlap by three days.",
        },
...
def test_today_live_data_feels_like_a_financial_command_center():
    """Catches regressions back to an amount-only engineering summary."""
    with pytest.importorskip("playwright.sync_api").sync_playwright() as pw:
        browser = pw.chromium.launch()
        context, page = _authed_page(browser)
        page.route("**/api/meridian/today", _fulfill(_live_today_payload()))
        _open_workspace(page, "today")

        assert page.locator("[data-sts-figure]").inner_text() == "$684.00"
        ...
        assert "Projected balance" in forecast.get_attribute("aria-label")
        assert page.locator("[data-forecast-label]").count() == 5

        metrics = page.locator("[data-supporting-metric]")
        assert metrics.count() == 3
        assert "$2,460.00" in metrics.nth(0).inner_text()
        assert "$1,776.00" in metrics.nth(1).inner_text()
        assert "17 days" in metrics.nth(2).inner_text()

        assert "Verizon rose by $11" in page.locator("[data-beacon-summary]").inner_text()
        assert page.locator("[data-today-brief]").is_visible()
        assert "insurance renewal" in page.locator("[data-brief-summary]").inner_text()

        assert not page.locator("#advisor-fab").is_visible()
        page.locator("[data-today-brief] [data-open-advisor]").click()
        assert page.locator("#advisor-panel").is_visible()
        browser.close()
