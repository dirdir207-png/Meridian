...
# observed per-bill figure, one derived even split, one that states nothing, and one
# legacy payload carrying a bare number with no basis.
MODEL = {
    ...
    "events": [
        ...
        },
        {
            ...
            "reserved": {"minor": 6500, "currency": "USD"},
            "fundingBasis": "derived",
            "fundingBasisDivisor": 3,
            "fundingAttribution": "meridian",
            "fundingObservedAt": "2026-09-08T13:42:00Z",
            "fundingSource": None,
            "source": "crew",
            "observedAt": None,
...
def test_the_stated_reserve_renders_with_its_author_and_an_unlabelled_one_does_not(dial_page):
    ...
    # The derived figure is stated as Meridian's estimate and never as Crew's, on the row
    # itself -- D-013 requires the label wherever the figure is stated, not only in the
    # ticket, or the row would read as an observed reservation.
    internet = _meta_for(page, "Internet")
    assert "set aside" in internet
    assert "Meridian estimate" in internet
    assert "Crew" not in internet
...
def test_the_ticket_names_the_bill_level_author_and_the_derived_divisor(dial_page):
    page, _errors = dial_page
    ...
    derived_rows = page.locator(".obs-evidence-ticket .obs-ticket-rows").inner_text().lower()
    assert "meridian estimate, split across 3 bills" in derived_rows
    assert "crew" not in derived_rows

    # The centre readout carries the same statement with the occurrence it belongs to.
    status = page.locator(".obs-dial-center-status").inner_text()
    assert status.startswith("Next due ")
    assert "Meridian estimate" in status
